...
11개 장 **61스텝**이 하나의 프로그램을 키운다 — 할 일 관리 CLI다. 장마다 다른 예제로 갈아타지 않는다.
앞 장의 결과물이 뒷 장의 재료이므로, 건너뛰면 뒷 장이 성립하지 않는다.

| 장 | 주제 | 상태 |
|---|---|---|
| 1 | 코드를 실행시키다 — Node 실행, 스크립트 파일, const | ✔ |
| 2 | 함수로 묶는다 — 선언·반환·조건·기본값·argv·종료 코드 | ✔ |
| 3 | 목록을 담는다 — 배열, 인덱스, push, 객체, 속성 변경 | ✔ |
| 4 | 훑고 고른다 — for-of, 블록 스코프, 화살표 함수, map, filter, find | ✔ |
| 5 | 파일을 나눈다 — export / import, default, 모듈 스코프 | ✔ |
| 6 | 잘못된 입력을 막는다 — throw, try/catch, 이름 붙은 에러, 가드 절 | ✔ |
| 7 | 기다렸다 이어간다 — Promise, async/await, fs/promises, 에러 전파 | ✔ |
| 8 | 틀을 만든다 — class, 생성자, 메서드, private 필드, getter | ✔ |
| 9 | 스스로 검증한다 — node --test, assert, 테스트 격리, 테스트 우선 | ✔ |
| 10 | 이력을 남긴다 — git init·gitignore·커밋·원격·충돌 해소·가지 | ✔ |
| 11 | 갈아 끼운다 — 전략·옵저버·커맨드·팩토리·모듈 싱글턴 | ✔ |

장 경계를 왜 이렇게 그었고 각 장을 무엇으로 판정하는지는 **[CURRICULUM.md](CURRICULUM.md)** 에 있다.
...
| scenario | 하는 일 | 추가 필드 |
|---|---|---|
| `ensure_remote` | 로컬 bare 저장소를 만들어 origin으로 등록한다 | — |
| `teammate_push` | 동료가 원격에 먼저 push해 둔 상황을 만든다 | `file`, `content`, `message`, `branch` |
| `seed_files` | 스텝의 전제가 되는 파일을 놓는다 (리팩터링할 레거시 코드 등) | `files: [{path, content}]` |

`seed_files`는 **이미 있는 파일을 덮어쓰지 않는다** — 학습자가 손으로 쓴 코드가 상황 연출에 밀려
사라지지 않도록, 있으면 건너뛰고 그 사실을 돌려준다. 심는 경로는 항상 새 경로여야 한다.

새 상황이 필요하면 `dojo/scenario.mjs`에 등록하고 `content/schema.json`의 `setup.scenario` enum에
추가한다 — **엔진 변경이므로 회귀 테스트를 동반한다.**
...
| [CURRICULUM.md](CURRICULUM.md) | 챕터 작성자 | 장 배치·누적 관계·판정 3단 사다리 |
